...
- при попытке создания новой заявки выполняется "INSERT ... ON CONFLICT DO NOTHING"
- если запись уже существует — выполняется поиск по user_id и idempotency_key. Возвращается существующая заявка
